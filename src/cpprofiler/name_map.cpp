...
namespace cpprofiler {
    ...
    static const Location& findLocation(const SymbolTable& st, const string& ident) {
    ...
    static string extractExpression(const vector<string>& model, const Location& loc) {
    ...
    std::string NameMap::replaceNames(const std::string& text, bool expand) const {
      ...
      const auto var_names_begin = std::sregex_iterator(text.begin(), text.end(), var_name_regex);
      ...
      for (auto it = var_names_begin; it != var_names_end; ++it) {
        ...
        if (match.position() != 0) {
          const auto non_id = text.substr(pos, static_cast<size_t>(match.position()) - pos);
    ...
    // static string replaceAssignments(const string& path, const string& epxression) {
    //   SymbolTable st;
    ...
    void NameMap::addIdExpressionToMap(const vector<string>& model, const string& ident) {
      debug("force") << "ident:" << ident << std::endl;

      const auto& loc = findLocation(id_map_, ident);
      // debug("force") << "location: " << loc << std::endl;

      if (loc.sl == 0) return; // default (empty) location?
